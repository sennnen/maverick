...
//
// Persisted via @AppStorage (UserDefaults), the same mechanism every other macOS Maverick preference uses.
// The Android side mirrors this exactly in Units.kt + MavPrefs.
...
/// How the Effort score is displayed (#268). Maverick's Effort is stored 0–100 (StrainScorer.maxStrain = 100);
...
enum UnitPrefs {
    ...
    /// resolves to `.hundred` (Maverick's native axis). Mirrored on Android by MavPrefs("effort.scale").
    ...
    /// Resolve the stored Effort-scale raw value, defaulting to Maverick's native 0–100 axis.
...
enum UnitFormatter {
    ...
    /// Maverick stores Effort 0–100 (StrainScorer.maxStrain = 100). WHOOP's Day Strain axis is 0–21, and
