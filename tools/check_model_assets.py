...
def ios_target_failures(root: Path, models: list[dict]) -> list[str]:
    """Every admitted package, and the Swift that loads it, must reach the Xcode target.

    Sitting in `apps/ios/Maverick/Models/` is not the same as shipping. This check exists because
    the project once listed the ECG model and nothing else: all forty-one zoo packages were on
    disk, hashed by the check above, and absent from the app. The iOS tests could not have caught
    it either, because the test target did not compile the runner that would have loaded them.

    It used to read the committed `project.pbxproj`. That file is generated by XcodeGen and no
    longer committed — it could not be reproduced identically outside a fully built tree — so the
    question is asked of the spec instead, and asked more directly: the app target takes the whole
    `Maverick` directory as a source path, so a package inside it is in the target by construction.
    What can still go wrong is the file not being there, or the spec no longer globbing the
    directory, and both are checked here.
    """
    spec = root / "apps/ios/project.yml"
    if not spec.is_file():
        return [f"iOS project spec is missing: {spec.relative_to(root)}"]
    failures: list[str] = []

    # The glob the rest of this depends on. Without it every check below proves nothing.
    text = spec.read_text(encoding="utf-8", errors="replace")
    if "- path: Maverick\n" not in text:
        failures.append(
            "apps/ios/project.yml no longer takes the whole Maverick directory as a source path, "
            "so being on disk no longer means being in the target"
        )

    models_dir = root / "apps/ios/Maverick/Models"
    for model in models:
        name = f"{model['model']}.mlpackage"
        if not (models_dir / name).is_dir():
            failures.append(f"iOS bundle is missing {name}; it is admitted and not shipped")

    sources = {path.name for path in (root / "apps/ios/Maverick").rglob("*.swift")}
    for source in IOS_ZOO_SOURCES:
        if source not in sources:
            failures.append(f"iOS target has no {source}, so no model can be loaded")
    return failures
